...
export async function generateGrowthPlugin(
  ...
  const primaryDimension = dimensions.find(d => d.confidence > 0.8)?.dimension || dimensions[0]?.dimension || 'planning';
  ...
  const growthPluginItems: GrowthPluginItem[] = growthPluginArray.map((item: unknown, idx: number) => {
    ...
    const howToImprove = Array.isArray(howToImproveRaw) 
      ? howToImproveRaw.map(String) 
      : [];

    return {
      areaName: String(it.areaName ?? ''),
      relevanceScore: typeof it.relevanceScore === 'number' ? it.relevanceScore : 0,
      whatNeedsImprovement: String(it.whatNeedsImprovement ?? ''),
      whyThisMatters: String(it.whyThisMatters ?? ''),
      howToImprove,
    };
